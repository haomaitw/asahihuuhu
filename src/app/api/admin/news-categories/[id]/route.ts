...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const snap = await adminDb.collection('news-categories').doc(id).get()
    if (!snap.exists) return NextResponse.json({ error: 'Not found' }, { status: 404 })
    await adminDb.collection('news-categories').doc(id).update({ ...body, updatedAt: new Date().toISOString() })
    const updated = await adminDb.collection('news-categories').doc(id).get()
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  try {
    const snap = await adminDb.collection('news-categories').doc(id).get()
    if (!snap.exists) return NextResponse.json({ error: 'Not found' }, { status: 404 })
    await adminDb.collection('news-categories').doc(id).delete()
