...
export async function GET() {
  ...
  try {
    const snap = await adminDb.collection('news-categories').orderBy('createdAt', 'desc').get()
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const ref = await adminDb.collection('news-categories').add({ ...body, createdAt: now, updatedAt: now })
