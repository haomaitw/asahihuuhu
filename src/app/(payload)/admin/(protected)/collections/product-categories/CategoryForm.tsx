...
export function CategoryForm({ initialData, isEdit }: { initialData?: any; isEdit?: boolean }) {
  ...
  React.useEffect(() => {
    if (!isEdit || !id) return
    fetch(`/api/admin/product-categories/${id}`, { credentials: 'include' })
      .then((r) => r.json())
  ...
  const handleSave = async () => {
    ...
    try {
      const r = await fetch(
        isEdit ? `/api/admin/product-categories/${id}` : '/api/admin/product-categories',
        {
          method: isEdit ? 'PATCH' : 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({ slug: form.slug, order: form.order, name: form.name }),
        }
      )
      if (!r.ok) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await fetch(`/api/admin/product-categories/${id}`, { method: 'DELETE', credentials: 'include' })
      toast.success('已刪除')
      router.push('/admin/collections/product-categories')
      router.refresh()
